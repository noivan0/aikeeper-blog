...
【본문 길이】 목표 3,000~3,500자 (공백 제외 최소 3,000자 필수)
...
3. 쿠팡 링크 — 각 상품 섹션에서 아래 형식으로 반드시 삽입 (텍스트와 링크를 한 줄에 함께, 정확히 이 형식):
   지금 쿠팡에서 확인하기 → [URL]
   (예: "지금 쿠팡에서 확인하기 → https://link.coupang.com/a/xxxx")
   각 상품당 2회 삽입 (소개 후 1회, 후기 후 1회):{link_rules}
4. 이미지 배치 — 본문에 [IMAGE] 마커로 이미지 위치를 지정할 것:
   - 각 상품 소개 직후 "[IMAGE]" 한 줄 삽입
   - 텍스트 500자마다 최소 1개 이미지 위치 지정
   - 이미지와 텍스트가 자연스럽게 이어지도록 전후 문맥 연결
...
(빈 줄)
④ [상품1 소제목] 🛒 상품명 — 가격의 특징
   상품 소개 + 첫인상 120~160자 (구체적 수치 포함)
   지금 쿠팡에서 확인하기 → [링크1URL]        ← 여기서 이미지 삽입됨
   장점 + 단점 솔직한 후기 120~160자
   지금 쿠팡에서 확인하기 → [링크1URL]        ← 여기서 OG카드 삽입됨
(빈 줄)
⑤ [상품2 소제목] 🛒 상품명 — 가격의 특징
   상품 소개 + 첫인상 120~160자
   지금 쿠팡에서 확인하기 → [링크2URL]        ← 이미지
   장점 + 단점 솔직한 후기 120~160자
   지금 쿠팡에서 확인하기 → [링크2URL]        ← OG카드
(빈 줄)
⑥ (상품3 있으면 동일)
⑦ ✅ 예산별 선택 가이드
   비교 정리 3~5줄 (어떤 분께 어떤 상품이 맞는지 명확히)
(빈 줄)
⑧ 마무리 1~2줄 + 추천 대상 명시
(빈 줄)
⑨ 해시태그 4~6개 (없어도 됨, 과도한 해시태그는 스팸 신호)

★ 핵심 규칙: 링크 전후로 반드시 120자 이상 텍스트가 있어야 함
★ 링크를 두 줄 연속으로 쓰지 말 것 (반드시 텍스트 사이에 배치)

[분량] 3,000~3,500자 (공백 제외 최소 3,000자 필수)
