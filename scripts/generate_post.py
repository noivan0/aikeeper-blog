...
try:
    from search_autocomplete import get_search_keywords as _get_seo_kw
    _SEO_TITLE_AVAILABLE = True
except ImportError:
    def _get_seo_kw(kw, pname=""): return []
    _SEO_TITLE_AVAILABLE = False
"""
...
def generate_post(topic: str, keywords: list = None, angle: str = "") -> dict:
    ...
    if _SEO_TITLE_AVAILABLE:
        try:
            _kws = _get_seo_kw(topic, topic)[:8]
            if _kws:
                _kw_list = "\n".join(f"  - {k}" for k in _kws)
                seo_title_hint = f"【실제 검색어 (구글 자동완성 기반)】\n{_kw_list}\n→ 위 검색어를 제목에 1개 이상 자연스럽게 포함하세요."
...
[SEO 검색 자동완성 기반 제목 최적화]
{seo_title_hint}

===TITLE===
SEO 최적화 제목 (40자 이내, 이모지 절대 사용 금지, 큰따옴표 절대 사용 금지)
[핵심] 위 실제 검색어를 1개 이상 제목에 자연스럽게 포함할 것 — 검색 유입 극대화
...
def make_seo_slug(title: str, topic: str) -> str:
    ...
    ko_brand_map = {
        ...
        '한국어': 'korean', '국내': 'korea', '한국': 'korea', '기업': 'enterprise', '스타트업': 'startup',
        '뉴스': 'news', '오늘': 'today', '주요': 'top', '최신': 'latest',
        '브리핑': 'briefing', '정리': 'summary', '총정리': 'roundup',
        '경제': 'economy', '정치': 'politics', '사회': 'society', '국제': 'global',
        '에너지': 'energy', '전기요금': 'electricity', '물가': 'prices',
        '관세': 'tariff', '수출': 'export', '무역': 'trade',
