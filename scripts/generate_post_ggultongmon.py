...
- 가격/스펙/장단점 구체적으로 비교
- 검색의도 키워드 자연스럽게 포함
- 분량: 최소 7,000자 이상 (7,000~9,000자 목표, 구글 SEO Helpful Content 기준)
...
## 글 품질 강화 지침 (구글 SEO Helpful Content Update 2024 기준)
- 각 상품 섹션: 실제 구매자 입장의 장단점 솔직하게 작성 (단점 반드시 2개 이상, 표면적 설명 금지)
...
- 분량: 최소 7,000자 이상 (7,000~9,000자 목표)
- 각 상품 섹션: 최소 600자 이상 (장점 3개 각 2줄, 단점 2개 각 2줄, 실사용 시나리오 2가지)
- 비교표 아래: "케이스별 구매 가이드" 섹션 필수 (A/B/C 케이스 3개, 각 200자 이상)
- 해당 카테고리 전문 지식 기반 "올바른 사용법/관리법" 섹션 추가 (300자 이상)
- E-E-A-T 강화: 구체적 수치/날짜/사용 기간 반드시 포함 ("2026년 4월 기준", "실제로 3주 사용 후")
...
## 포스트 구조 (반드시 준수, 총 7,000자 이상)
1. 도입부 훅 (300~400자): 독자가 공감할 구체적 상황으로 시작
2. 선택 기준 (h2, 400자 이상): 구매 시 체크할 핵심 포인트 4~5개, 각 항목 설명 2줄 이상
3. 각 상품별 h2 섹션 (상품당 600자 이상):
   - h2 제목 직후 반드시 [PRODUCT_N] 마커 한 줄
   - 실제 사용 경험 기반 상세 설명 (장점 3개 각 2줄, 단점 2개 각 2줄)
   - 실사용 시나리오 2가지 (어떤 상황의 누가 쓰면 좋은지)
   - 섹션 끝에 [BUY_N] 마커 한 줄
4. 비교표 (h2): 가격/특징/배송/추천대상 4열 이상, 추천 상품 강조 행 표시
5. 케이스별 구매 가이드 (h2, 600자 이상): A/B/C/D 케이스 4개, 각 150자 이상
   - 예산별 / 목적별 / 상황별로 구분
6. 올바른 사용법 & 관리법 (h2, 300자 이상): 카테고리 전문 지식 기반 팁 3개 이상
7. FAQ (h2, 답변 각 200자 이상): 실제 구글 검색창에 치는 구어체 질문 5개
   - "~는 얼마인가요?" (가격 검색 → 고CPC 유도)
   - "~와 ~의 차이는?" (비교 검색 → 고CTR)
   - "~하면 부작용/단점 있나요?" (우려 해소 → 전환율 높음)
   - "~를 사면 후회하나요?" (구매 결정 직전 검색)
   - "~하면 어떻게 되나요?" (사용법/효과 검색)
   - 각 답변 200자 이상
8. 마무리 (h2, 250자 이상): 한 줄 정리 + 구체적 구매 권유 + 다음 글 예고
9. 관련 검색어 마커 (본문 최하단): 아래 형식으로 반드시 1줄 추가
...
"""
    ...
    for _retry in range(2):
        ...
        retry_prompt = (
            ...
            "- 각 상품 설명을 최소 600자 이상으로 확장 (장점 3개 각 2줄, 단점 2개 각 2줄)\n"
            "- 실제 사용 시나리오 2가지 추가 (어떤 상황의 누가 쓰면 좋은지)\n"
            "- '케이스별 구매 가이드' h2 섹션 추가 (A/B/C/D 케이스 4개, 각 150자 이상)\n"
            "- '올바른 사용법 & 관리법' h2 섹션 추가 (팁 3개 이상, 300자)\n"
            "- FAQ를 5개 이상, 각 답변 200자 이상\n"
            "- 비교표 아래 케이스별 구매 결정 가이드 추가\n"
            "- 마무리 섹션 250자 이상으로 확장 (구매 권유 + 다음 글 예고)"
        )
        _raw = ""
        with client.messages.stream(
